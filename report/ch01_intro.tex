...
\noindent\textbf{结构：}第\ref{ch:survey}章综述研究现状；第\ref{ch:model}章形式化系统与最优检测；第\ref{ch:theory}章给 RKHS/MKL 理论；第\ref{ch:method}章给方法；第\ref{ch:impl}章给实现；第\ref{ch:exp}章给实验；第\ref{ch:discuss}章给讨论与结论。
